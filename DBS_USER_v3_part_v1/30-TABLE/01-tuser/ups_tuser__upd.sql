...
DROP PROCEDURE IF EXISTS ups_tuser__upd;

DELIMITER $$
CREATE PROCEDURE ups_tuser__upd (
  IN  p_json JSON,  
  OUT r_json JSON
)
BEGIN
  ...
  main: BEGIN
    ...
    WHERE userid = v_p_userid
      AND NOT (name <=> v_p_name)
    ; 

    SET v_ret_val1 = ROW_COUNT();
    
    IF v_ret_val1 = 0 THEN
      SELECT COUNT(*) INTO v_ret_val2
      FROM tuser
      WHERE userid = v_p_userid
      ;
      IF v_ret_val2 = 0 THEN
        SET v_ret_mssg = CONCAT('FAIL: unknown userid [', v_p_userid, ']');
      ELSE
        SET v_ret_mssg = 'SUCC: no row updated';
      END IF;
    ELSE
      SET v_ret_val2 = v_ret_val1;
      SET v_ret_mssg = CONCAT('SUCC: updated [', v_p_userid, ']');
    END IF; 
  END main;
