...
DROP PROCEDURE IF EXISTS ups_tuser__ins;

DELIMITER $$
CREATE PROCEDURE ups_tuser__ins (
  IN  p_json JSON,  
  OUT r_json JSON
)
BEGIN
  DECLARE v_p_userid VARCHAR(32);
  DECLARE v_p_name   VARCHAR(64);
  DECLARE v_p_userid_MINLEN TINYINT DEFAULT 4;
  DECLARE v_p_name_MINLEN   TINYINT DEFAULT 2;
  
  DECLARE v_dtnow  DATETIME(6);
  ...
  main: BEGIN
    ...
    IF IFNULL(JSON_CONTAINS_PATH(p_json, 'all', '$.userid', '$.name'), 0) = 0 THEN
      SET v_ret_val1 = -201;
      SET v_ret_val2 = 1;
      SET v_ret_mssg = 'ERROR: required keys missing (need userid, name)';
      LEAVE main;
    ELSE
      SET v_p_userid = IFNULL(UPPER(TRIM(JSON_VALUE(p_json, '$.userid'))), ''); 
      ...
      SET v_p_name   = IFNULL(UPPER(TRIM(JSON_VALUE(p_json, '$.name'  ))), '');
      IF CHAR_LENGTH(v_p_name) < v_p_name_MINLEN THEN
        SET v_ret_val1 = -201;
        SET v_ret_val2 = 3;
        SET v_ret_mssg = CONCAT('ERROR: name must be at least ', v_p_name_MINLEN, ' characters long');
        LEAVE main;
      END IF;
    END IF;
    ...
    SET v_dtnow  = NOW(6);
    INSERT INTO tuser (userid, name, dt7, dt8)
      VALUES (v_p_userid, v_p_name, v_dtnow, v_dtnow);

    SET v_ret_val1 = ROW_COUNT();
    
    IF v_ret_val1 = 0 THEN
      SET v_ret_mssg = 'FAIL: no row inserted';
    ELSE
      SET v_ret_mssg = CONCAT('SUCC: inserted [', v_p_userid, ']');
    END IF; 
  END main;
